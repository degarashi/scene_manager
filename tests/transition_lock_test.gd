...
func test_concurrent_switch_to_scene_is_locked() -> void:
	...
	SceneManager.call_deferred("switch_to_scene", Scenes.Id.SCENE_0, false, opts1)
	...
	var node2 = await SceneManager.switch_to_scene(Scenes.Id.SCENE_1, false, opts2)
	...
	assert_int(SceneManager._current_scene_enum).is_equal(Scenes.Id.SCENE_0)
