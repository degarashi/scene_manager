...
## Scene Enum
The enum `Scenes.Id` is auto-generated by the `manager.gd` script in the `scenes.gd` file as scenes are loaded and named in the tool view. Note there will always be a `NONE` enum to specify no scene. The location of the `scenes.gd` file can be specified in the Project Settings. 
...
The `Scenes.Id` can also be set as an export property in the editor inspector using the `SceneResource` class. 
...
## Loading Scenes
```gdscript
func load_scene(scene: Scenes.Id, load_options: SceneLoadOptions = create_load_options()) -> void:
```

`load_scene` is the main function call in `SceneManager` for loading scenes into the tree. It takes an enum `Scenes.Id` for the scene to load and an optional parameter `load_options` which is a class of parameters in `SceneLoadOptions`. If not provided, it will use the defaults in the class.
