...
func add_to_subsection(scene_address: String, section_name: String, sub_section: String) -> void:
	for key in scenes:
		if scenes[key][SceneKey.PATH] == scene_address:
			scenes[key]["settings"][section_name]["subsection"] = sub_section
			break


## Removes the scene from the specified subsection.
func remove_from_subsection(
	scene_address: String, section_name: String, _sub_section: String
) -> void:
	for key in scenes:
		if scenes[key][SceneKey.PATH] == scene_address:
...
func remove_section(section_name: String) -> void:
	...
	for key in scenes:
		if scenes[key][SceneKey.SECTIONS].has(section_name):
			scenes[key][SceneKey.SECTIONS].erase(section_name)
...
func add_scene_to_section(scene_address: String, section_name: String) -> void:
	...
	for key in scenes:
		if scenes[key][SceneKey.PATH] == scene_address:
			if section_name not in scenes[key][SceneKey.SECTIONS]:
				scenes[key][SceneKey.SECTIONS].append(section_name)
			break
...
func save() -> void:
	# To prevent writing to the file when not necessary,
	# compare the dictionary against the stored file content
	if not has_changes:
...
static func _load_file() -> Dictionary:
	...
	if FileAccess.file_exists(file_path):
		...
		while not file.eof_reached():
			...
			if line == SceneData.END_DICTIONARY:
				in_dictionary = false
			elif line == SceneData.DICTIONARY:
				in_dictionary = true
...
func _get_scenes_helper(root_path: String) -> Dictionary:
	...
	if dir:
		# TODOGODOT4 fill missing arguments https://github.com/godotengine/godot/pull/40547
		dir.list_dir_begin()
...
func _remove_scenes(path: String) -> void:
	for scene in self.scenes.keys():
		if self.scenes[scene][SceneKey.PATH].begins_with(path):
...
func _refresh_scenes() -> void:
	...
	for key in self.scenes:
		data_files[self.scenes[key][SceneKey.PATH]] = key
	...
	for address in scenes_to_add:
		var new_data := {SceneKey.SECTIONS: [], SceneKey.PATH: address}
